...
class FareCalculator:
    ...
    def fetch_intermediate_stops_with_gps(self, start_stop_code, end_stop_code):
        query = """
            SELECT route_id FROM stops WHERE code = %s
            INTERSECT 
            SELECT route_id FROM stops WHERE code = %s
        """
        route_id_result = self.execute_query(query, (start_stop_code, end_stop_code))
        ...
        query = """
            ...
            WHERE 
                s.route_id = %s AND
                s.sequence_in_route > (SELECT sequence_in_route FROM stops WHERE code = %s AND route_id=s.route_id) AND
                s.sequence_in_route < (SELECT sequence_in_route FROM stops WHERE code = %s AND route_id=s.route_id)
            ORDER BY 
                s.sequence_in_route
        """
        intermediate_stops = self.execute_query(query, (route_id, start_stop_code, end_stop_code))

        query = "SELECT stop_details FROM routes WHERE route_id = %s"
        route_data_result = self.execute_query(query, (route_id,))
        stop_details = json.loads(route_data_result[0][0])

        result = []
        stop_number = 1
        for name, code, sequence in intermediate_stops:
