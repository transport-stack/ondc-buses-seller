...
class TestSelect(unittest.TestCase):
    ...
    def test_transaction_cache_data_found(self):
        ...
        print(f'\n-Testing transaction cache data found for ID: {transaction_id} \n Result: {cache_data is not None}')
        self.assertIsNotNone(cache_data, self.response_val("transaction_cache_data_found", "No data found for the given transaction ID"))

    def test_item_id_present(self):
        item_id = self.request_data.get("message", {}).get("order", {}).get("items")[0].get("id")
        print(f'\n-Testing if item ID is present: \n Result: {item_id is not None}')
    ...
    def test_provider_id_present(self):
        ...
        print(f'\n-Testing if provider ID is present: \n Result: {provider_id is not None}')
        self.assertIsNotNone(provider_id, self.response_val("provider_id_present", "Invalid request format, Missing key: item_id/provider_id"))
    ...
    def test_selected_item_present_in_cache(self):
        ...
        print(f'\n-Testing selected item present in cache: \n Item ID: {item_id}, Cached Item IDs: {cached_item_ids}')
