...
from .cryptic_utils import create_authorisation_header, verify_authorisation_header
...
def post_request(url, data, *args):
    try:
        ...
        # logging.info(f"Request body:================= {request_body_raw_text is not None}")
        ...
        post_result = requests.post(url, json=request_body_raw_text, headers=headers)
        logging.info(f"post_result=========: {post_result}===={post_result.status_code}======={post_result.text}")
        post_result.raise_for_status()

        return post_result.status_code, post_result.json()
