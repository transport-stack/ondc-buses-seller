...
from ondc_protocol.models import ONDCDomain, ONDCAction
...
def send_search_payload_to_client(message):
    print(f"retail search payload: {message}")
    search_message_id = message['message_ids']['search']
    search_payload = get_first_ondc_request(ONDCDomain.RETAIL, ONDCAction('search'),
                                            search_message_id)
    resp, return_code = make_retail_search_payload_request_to_client(search_payload)
    print(f"Got response {resp} from client with status-code {return_code}")


def send_search_response_to_gateway(message):
    print(f"retail on_search payload: {message}")
    on_search_message_id = message['message_ids']['on_search']
    on_search_payload = get_first_ondc_request(ONDCDomain.RETAIL,
                                               ONDCAction('on_search'),
                                               on_search_message_id)
    gateway_or_bap_endpoint = fetch_gateway_url_from_lookup()
    status_code = make_request_over_ondc_network(on_search_payload,
                                                 gateway_or_bap_endpoint, 'on_search')
    print(f"Sent responses to bg/bap with status-code {status_code}")
