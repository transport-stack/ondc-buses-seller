...
def send_bpp_responses_to_bg_or_bpp(message):
    print(f"{message['request_type']} payload: {message}")
    ...
    status_code = make_request_over_ondc_network(client_responses,
                                                 gateway_or_bap_endpoint,
...
def dump_request_payload(request_payload, domain, action=None):
    ...
    is_successful = add_ondc_request(domain=ONDCDomain(domain),
                                     action=ONDCAction(action), message_id=message_id,
...
def send_logistics_on_call_count_to_client(message, request_type="on_search"):
    print(f"logistics {request_type} payload: {message}")
